...
var $author$project$Style$monospace = $mdgriffith$elm_ui$Element$Font$family(
	_List_fromArray(
		[
			...
			{name: 'Space Mono', url: 'https://fonts.googleapis.com/css2?family=Space+Mono:ital,wght@0,400;1,700&display=swap'}),
		...
		A2(
			...
			function (x) {
				if (x.$ === 'Mosaic') {
					var items = x.a;
					return viewMasonry(
						{columns: 2, items: items, padding: 0, spacing: 10, viewItem: renderItem});
				} else {
					var item = x.a;
			...
			[
				...
				_List_fromArray(
					[
						...
						$author$project$Pages$SmartmarkInspiration$Top$viewColumn(model.data)
					]))
			]),
		title: 'Homepage'
	};
